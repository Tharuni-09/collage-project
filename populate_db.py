...
# Connect to database using absolute path to avoid directory issues
...
# Insert specific HOD user: Venu Gopal (UID 39)
cursor.execute("""
    INSERT INTO users (uid, username, password_hash, name, role, email, phone, is_hod)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
""", (39, 'venugopal', password_hash, 'Venu Gopal', 'faculty', 'venugopal@example.com', '9876540001', 1))
...
student_count = 0
faculty_count = 0
faculty_count += 1 # Including the HOD inserted above
...
for i in range(120):
    ...
    if i < 100:  # First 100 are students
        ...
        department = random.choice(departments)
        # Make the first faculty member an HOD for testing purposes
        is_hod = 0 # Other faculty members are standard

        # Insert user
        cursor.execute("""
            INSERT INTO users (uid, username, password_hash, name, role, email, phone, is_hod)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (user_id, username, password_hash, f"Dr. {full_name}", role, email, f"987655{str(user_id-100).zfill(4)}", is_hod))
